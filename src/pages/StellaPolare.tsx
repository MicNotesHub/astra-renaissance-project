...
export default function StellaPolare() {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      const {
        data,
        error
      } = await supabase.from("Stella_Polare").select("*").order("created_at", {
        ascending: false
      });
      if (error) {
  ...
  return <div className="min-h-screen bg-background">
      ...
      <section className="relative h-[50vh] min-h-[400px] flex items-center justify-center overflow-hidden pt-16">
        <img src={stellaPolareHero} alt="Stella Polare" className="absolute inset-0 w-full h-full object-cover object-[center_30%]" />
      ...
      <section className="py-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          {loading ? <div className="flex justify-center py-12">
            ...
            </div> : articles.length === 0 ? <p className="text-center text-muted-foreground">
              Nessun articolo disponibile al momento.
            </p> : <motion.div initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          delay: 0.3
              ...
              {articles.map((article, index) => <motion.a key={article.id} href={article.URL || "#"} target="_blank" rel="noopener noreferrer" initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            delay: 0.4 + index * 0.1
                  ...
                  <Button size="lg" className="w-full gap-3 px-8 py-6 text-lg font-semibold">
